...
import { Package, Save, Edit2, RotateCcw, Plus, Trash2, Camera } from 'lucide-react';
        ...
        <div style={{ padding: '20px', height: '100%', overflowY: 'auto' }}>
            ...
            {isAddOpen && (
                <div className="glass-card" style={{ marginBottom: '20px', padding: '20px', background: '#f9f9f9', border: '1px solid #ddd' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 100px 150px 150px auto', gap: '10px', alignItems: 'end' }}>
                        <div>
                        ...
                        {products.filter(p => !p.parent_id).map(main => (
                            <React.Fragment key={main.id}>
                                <tr style={{ borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
                                    <td style={{ padding: '15px' }}>{main.code}</td>
                                    <td style={{ padding: '15px', fontWeight: 'bold' }}>{main.name}</td>
                                    <td style={{ padding: '15px' }}>{main.price}</td>
                                    ...
                                    <td style={{ padding: '15px' }}>
                                        {editingId === main.id ? (
                                            ...
                                            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                                <button onClick={() => handleEdit(main)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--secondary-color)' }} title="Editar"><Edit2 size={18} /></button>
                                                <button onClick={() => { setAddingVariantToId(main.id); setInlineVariantForm({ name: '', price: main.price, quantity: 0, expiration_date: '' }); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#28a745' }} title="Agregar Variante"><Plus size={18} /></button>
                                                <label style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', color: '#007bff' }} title="Subir Imagen">
                                                    <Camera size={18} />
                                                    <input type="file" accept="image/*" style={{ display: 'none' }} onChange={(e) => handleImageUpload(main, e)} />
                                                </label>
                                                <button onClick={() => handleDelete(main.id, main.name)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#dc3545' }} title="Eliminar"><Trash2 size={18} /></button>
                                            </div>
                                        )}
                                    </td>
                                </tr>
                                {addingVariantToId === main.id && (
                                    <tr style={{ borderBottom: '1px solid rgba(0,0,0,0.05)', background: '#eefaee' }}>
                                        <td style={{ padding: '15px', paddingLeft: '40px' }}>↳ Nueva</td>
                                        <td style={{ padding: '15px' }}>
                                ...
                                {products.filter(v => v.parent_id === main.id).map(variant => (
                                    <tr key={variant.id} style={{ borderBottom: '1px solid rgba(0,0,0,0.05)', background: '#fafafa' }}>
                                        <td style={{ padding: '15px', paddingLeft: '40px' }}>↳ {variant.code}</td>
                                        <td style={{ padding: '15px' }}>{variant.name}</td>
                                        <td style={{ padding: '15px' }}>{variant.price}</td>
                                        <td style={{ padding: '15px' }}>
                                        ...
                                        <td style={{ padding: '15px' }}>
                                            {editingId === variant.id ? (
                                                ...
                                                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                                    <button onClick={() => handleEdit(variant)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--secondary-color)' }} title="Editar"><Edit2 size={18} /></button>
                                                    <label style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', color: '#007bff' }} title="Subir Imagen">
                                                        <Camera size={18} />
                                                        <input type="file" accept="image/*" style={{ display: 'none' }} onChange={(e) => handleImageUpload(variant, e)} />
                                                    </label>
                                                    <button onClick={() => handleDelete(variant.id, variant.name)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#dc3545' }} title="Eliminar"><Trash2 size={18} /></button>
                                                </div>
                                            )}
                                        </td>
                                    </tr>
